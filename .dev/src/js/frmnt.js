document.addEventListener("DOMContentLoaded", function () {
	...
	function atualizarValor(event) {
		...
		const posX =
			(event.type === "mousemove" || event.type === "mousedown" ? event.clientX : event.touches[0].clientX) -
			container.getBoundingClientRect().left -
			posicaoInicialSinalizador
		...
		let valorAtual = ((valorMaximo - valorMinimo) * porcentagem) / 100 + valorMinimo
		valorAtual = valorMinimo + ((valorAtual - valorMinimo) / 15) * 15
	...
	function atualizarBarra() {
		...
		let valorAtual = parseFloat(valor.value)
	...
	const valor = document.getElementById("valor")
	valor.addEventListener("change", atualizarBarra)
})
...
const labelsTwo = document.querySelectorAll(".simulador__form--input.jhonson label")
labelsTwo.forEach((label) => {
	// Correção: substituir "labels" por "labelsTwo"
	const inputTwo = label.querySelector("input")
	label.addEventListener("click", () => {
